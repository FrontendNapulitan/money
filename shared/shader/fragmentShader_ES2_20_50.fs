...
uniform sampler2D uTexEdges_value1;
uniform sampler2D uTexEdges_value2;
...
// 	Sample the input pixels
    ...
    vec4 maskValues1 = texture2D(uTexEdges_value1, texCoord);  
    vec4 maskValues2 = texture2D(uTexEdges_value2, texCoord);  
...
// Silvery stripe or foil
	if (mask_Emerald_Foil.b > 0.2 && mask_Image_Window.b < 0.25) {
		...
		color.rgb = diffuseTex.rgb * directionalLightColor * diffuse * 0.2 + specular * 0.8 + color.rgb * 0.0;
	}
...
// Portrait window front
	...
	// 	Create a radial rainbow
		...
		float strength_for_current_position = gaussian(normal_loc, position_of_line, 0.75, 0.12)*mask_Image_Window.g;
	...
	if (maskValues1.b > 0.25) {
		
		float shift = (maskValues1.b) / 3.0;		
		vec3 specular = texture2D(metalMap, vec2(safeUTilt * -1.1 / halfPI+safeURot * 0.1 / halfPI, 0.5)).rgb;
		...
		float position_of_line = sigmoid(safeURot+0.25, 0.0, 2.0, 1.25, 1.5);
		float strength_for_current_position = gaussian(normal_loc, position_of_line, 0.5, 0.1);
	...
	if (maskValues2.b > 0.25) {
		...
		//sigmoid(float x, float mean, float std, float zeroPoint, float amplitude) {
		float position_of_line = sigmoid(safeURot-1.0, 0.0, 2.0, 1.25, 1.5);
		float strength_for_current_position = gaussian(normal_loc, position_of_line, 0.5, 0.1);
	...
	// 	The hologram with two opposing vertically moving bars is here
		...
		vec3 gray = vec3(0.5,0.5,0.5);
		color.rgb = diffuseTex.rgb * directionalLightColor * diffuse * 0.05 + (specular*0.5 + (rainbow * 0.6)) * 1.1+gray.rgb*(1.0-(specular*0.5 + (rainbow * 0.5)))*0.5;
	}
