...
// Silvery stripe or foil
	if (mask_Emerald_Foil.b > 0.4 && mask_Image_Window.b < 0.25) {
...
// Portrait window front
	if (mask_Image_Window.g > 0.25) {
		float shift = (mask_Image_Window.g) / 1.5;

		// 	Create a radial rainbow
		vec2 center = vec2(0.906, 0.72);
		...
		vec3 rainbow = texture2D(metalMap, vec2((safeUTilt * 1.1 + shift + (distance) * 2.0) / halfPI + safeURot * 0.1 / halfPI , 1)).rgb;
		...
		color.rgb = ((rainbow * 0.5 + specular * 0.8) * strength_for_current_position + color.rgb * (1.3 - strength_for_current_position));
